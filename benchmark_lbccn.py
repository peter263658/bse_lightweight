...
def benchmark_model(model_checkpoint, input_file, output_dir, num_runs=5, force_cpu=True):
    ...
    with torch.no_grad():
        warm = model(input_tensor)
        if isinstance(warm, tuple):
            warm = warm[0]          # 只留下增強後訊號
    ...
    for i in range(num_runs):
        ...
        with torch.no_grad():
            ...
            if isinstance(enhanced, tuple):      # ← 關鍵
                enhanced = enhanced[0]
    ...
    # --- 儲存增強後音訊 ------------------------------------------------------
    enhanced_audio = enhanced.squeeze(0).cpu().numpy().T  # [samples, channels]
    sf.write(os.path.join(output_dir, "enhanced_output.wav"),
            enhanced_audio, sr)
